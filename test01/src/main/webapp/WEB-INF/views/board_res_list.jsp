...
<script type="text/javascript">
	...
	function selChange() {
		...
		location.href = "search_board.do?nowPage=${paging.nowPage}&cntPerPage="
				+ sel + "&orderby=" + orderby + "&type=${type}&keyword=${keyword}";
	}
...
<body>
	...
	<div class="paging">
		<c:if test="${paging.startPage != 1}">
			<span
				onclick="location.href='search_board.do?type=${type}&keyword=${keyword}&nowPage=${paging.startPage - 1}&cntPerPage=${paging.cntPerPage}'">&lt;</span>
		</c:if>
		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="p">
			<c:choose>
				...
				<c:when test="${p != paging.nowPage}">
					<span
						onclick="location.href='search_board.do?type=${type}&keyword=${keyword}&nowPage=${p}&cntPerPage=${paging.cntPerPage}'">${p}</span>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.endPage != paging.lastPage}">
			<span
				onclick="location.href='search_board.do?type=${type}&keyword=${keyword}&nowPage=${paging.endPage+1}&cntPerPage=${paging.cntPerPage}'">&gt;</span>
		</c:if>
	</div>
</body>
</html>